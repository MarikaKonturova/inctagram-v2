...
export const ThemeSwitcher = memo(({ className, size = 25 }: ThemeSwitcherProps) => {
    ...
    return (
        <Button theme={'clear'} onClick={toggleTheme} style={{ lineHeight: 0 }} className={clsx('', {}, [className])}>
            {theme === Theme.DARK ? <DarkIcon height={size} width={size} /> : <LightIcon height={size} width={size} />}
        </Button>
    )
})
