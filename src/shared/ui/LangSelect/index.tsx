...
const languages: Record<string, JSX.Element> = {
  en: (
    <>
      <ENIcon className={cls.flagIcon} /> English
    </>
  ),
  ru: (
    <>
      <RuIcon className={cls.flagIcon} /> Russian
    </>
  ),
}
...
export const LangSelect = () => {
  ...
  return (
    <Listbox onChange={changeLangHandler} value={locale}>
      {({ open, value }) => (
        <div className={clsx(cls.container, { [cls.helper]: open })}>
          <Listbox.Button className={clsx(cls.button, cls.listItem)}>
            {languages[locale]}
            {open ? <ArrowUp className={cls.icon} /> : <ArrowDown className={cls.icon} />}
          </Listbox.Button>
          <Listbox.Options>
            {Object.entries(languages).map(
              ([key, label]) =>
                key !== value && (
                  <Listbox.Option className={cls.listItem} key={key} value={key}>
                    {label}
                  </Listbox.Option>
                )
            )}
